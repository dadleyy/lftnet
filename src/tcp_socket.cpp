...
TcpSocket::TcpSocket() : m_refcount(0), m_impl(0) { }

TcpSocket::TcpSocket(bool ssl) {
  if(ssl) m_impl = new impl::SslImpl();
  else m_impl = new impl::Impl();
  m_impl->m_refcount++;
};
...
int TcpSocket::Connect(const char *hostname, int port) {
  return m_impl != 0 ? m_impl->Connect(hostname, port) : -1;
};
...
SslImpl::SslImpl() : TcpSocket(), m_handle(socket(AF_INET, SOCK_STREAM, 0)), m_context(0) {
...
Impl::Impl() : TcpSocket(), m_handle(socket(AF_INET, SOCK_STREAM, 0)) {
}
